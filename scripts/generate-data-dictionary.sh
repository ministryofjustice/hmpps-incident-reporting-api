#!/usr/bin/env bash
#
# Exports the incident reporting schema as a flat CSV data dictionary.
#
# The descriptions come from the COMMENT ON statements in db/migration/V1_48__schema_comments.sql, so this
...
-- column of both. It bit the incentives API (see IR-1879), which has incentive_level_fkey on two
-- tables. pg_constraint carries the owning relation, so it cannot.
LEFT JOIN (
